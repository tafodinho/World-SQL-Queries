...
GROUP BY company, num having(num = 4 or num = 45)
...
-- table we can refer to stops by name rather than by number. Change the query so that the services 
-- between 'Craiglockhart' and 'London Road' are shown. If you are tired of these places try 'Fairmilehead'
--  against 'Tollcross' 
...
SELECT distinct  b.num, b.company, stopa.name, c.num, b.company
FROM route a JOIN route b ON
  (a.num=b.num) 
  JOIN route c ON
  (b.num= c.num)
...
WHERE stopb.name='Craiglockhart' or stopc.name = 'Lochend'